...
function InfoRow({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  ...
  return (
    <View style={{ flexDirection: 'row', alignItems: 'flex-start', paddingVertical: 10, gap: 12 }}>
      <View style={{ width: 32, height: 32, borderRadius: 10, backgroundColor: theme.iconBg, alignItems: 'center', justifyContent: 'center', marginTop: 1 }}>
        {icon}
      </View>
      <View style={{ flex: 1 }}>
        <Text style={{ fontSize: 11, fontWeight: '600', color: theme.textMuted, textTransform: 'uppercase', letterSpacing: 0.6, marginBottom: 2 }}>
          {label}
        </Text>
...
export const AccessDetailSheet = memo(function AccessDetailSheet({ guardian, onClose }: Props) {
  const theme = useTheme();
    ...
    >
      <View style={{ flex: 1, backgroundColor: theme.bg }}>
        ...
        <View style={{ alignItems: 'center', paddingTop: 12, paddingBottom: 4 }}>
          <View style={{ width: 36, height: 4, borderRadius: 2, backgroundColor: theme.cardBorder }} />
        </View>
        ...
        >
          <View>
            <Text style={{ fontSize: 11, fontWeight: '700', color: theme.textMuted, textTransform: 'uppercase', letterSpacing: 1.2, marginBottom: 2 }}>
              Détail accès
            </Text>
            <Text style={{ fontSize: 20, fontWeight: '800', color: theme.text, letterSpacing: -0.3 }}>
              {childName}
            </Text>
          </View>
          <TouchableOpacity
            onPress={handleClose}
            style={{ width: 36, height: 36, borderRadius: 12, backgroundColor: theme.iconBg, alignItems: 'center', justifyContent: 'center' }}
          >
        ...
        >
          {/* Avatar + statut */}
          <Animated.View entering={SlideInDown.delay(60).duration(350)} style={{ alignItems: 'center', marginBottom: 24 }}>
            <Avatar
          ...
          {/* Infos enfant */}
          <Animated.View entering={FadeInDown.delay(100).duration(300)} style={{ backgroundColor: theme.card, borderRadius: 18, borderWidth: 1, borderColor: theme.cardBorder, paddingHorizontal: 16, marginBottom: 12 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, paddingTop: 14, paddingBottom: 8 }}>
              <View style={{ width: 26, height: 26, borderRadius: 8, backgroundColor: theme.accentBg, alignItems: 'center', justifyContent: 'center' }}>
                <User size={13} color={theme.accent} strokeWidth={2} />
              </View>
              <Text style={{ fontSize: 12, fontWeight: '700', color: theme.textSecondary, textTransform: 'uppercase', letterSpacing: 0.8 }}>
                Enfant
              </Text>
            </View>
            <View style={{ height: 1, backgroundColor: theme.separator, marginBottom: 4 }} />

            <InfoRow
              icon={<User size={14} color={theme.textMuted} strokeWidth={2} />}
              label="Prénom & Nom"
              value={childName || '—'}
            />
            {child?.class_name ? (
              <InfoRow
                icon={<GraduationCap size={14} color={theme.textMuted} strokeWidth={2} />}
                label="Classe"
          ...
          {/* Infos autorisation */}
          <Animated.View entering={FadeInDown.delay(140).duration(300)} style={{ backgroundColor: theme.card, borderRadius: 18, borderWidth: 1, borderColor: theme.cardBorder, paddingHorizontal: 16, marginBottom: 12 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, paddingTop: 14, paddingBottom: 8 }}>
              <View style={{ width: 26, height: 26, borderRadius: 8, backgroundColor: theme.primaryBg, alignItems: 'center', justifyContent: 'center' }}>
                <Heart size={13} color={theme.primary} strokeWidth={2} />
              </View>
              <Text style={{ fontSize: 12, fontWeight: '700', color: theme.textSecondary, textTransform: 'uppercase', letterSpacing: 0.8 }}>
                Autorisation
              </Text>
            ...
            />
            <InfoRow
              icon={<Calendar size={14} color={theme.textMuted} strokeWidth={2} />}
          ...
          {/* Notes médicales */}
          {child?.medical_notes ? (
            <Animated.View entering={FadeInDown.delay(180).duration(300)} style={{ backgroundColor: theme.amberBg, borderRadius: 18, borderWidth: 1, borderColor: 'rgba(245,158,11,0.25)', paddingHorizontal: 16, marginBottom: 12 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, paddingTop: 14, paddingBottom: 8 }}>
                <View style={{ width: 26, height: 26, borderRadius: 8, backgroundColor: 'rgba(245,158,11,0.2)', alignItems: 'center', justifyContent: 'center' }}>
                  <AlertCircle size={13} color={theme.amber} strokeWidth={2} />
                </View>
                <Text style={{ fontSize: 12, fontWeight: '700', color: theme.amber, textTransform: 'uppercase', letterSpacing: 0.8 }}>
                  Notes médicales
                </Text>
              </View>
              <View style={{ height: 1, backgroundColor: 'rgba(245,158,11,0.2)', marginBottom: 12 }} />
              <Text style={{ fontSize: 14, color: theme.text, lineHeight: 22, paddingBottom: 16 }}>
                {child.medical_notes}
              </Text>
            </Animated.View>
          ) : null}
          ...
          {!isActive && (
            <Animated.View entering={FadeInDown.delay(220).duration(300)} style={{ backgroundColor: 'rgba(239,68,68,0.08)', borderRadius: 14, padding: 14, borderWidth: 1, borderColor: 'rgba(239,68,68,0.2)' }}>
              <Text style={{ color: '#ef4444', fontSize: 13, fontWeight: '600', lineHeight: 20 }}>
                Votre accès a été suspendu par le parent responsable. Contactez-le directement pour rétablir l'autorisation.
              </Text>
            </Animated.View>
          )}
        </ScrollView>

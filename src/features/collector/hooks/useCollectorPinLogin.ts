...
import { toAuthError } from '@/features/auth/utils/authError';
...
export function formatPinError(code: string): string {
  ...
  if (code === 'no_active_guardian')
    return 'Aucun accès actif pour cet email. Vérifiez avec le parent.';
...
export function useCollectorPinLogin(onVerified: (version: number) => void) {
  ...
  return useMutation({
    onMutate: () => {
      collectorPinLoginInProgress = true;
    },
    mutationFn: async ({
      email,
      pin,
    }: CollectorLoginPayload): Promise<CollectorLoginResponse> => {
    ...
    onSuccess: async data => {
      if (__DEV__)
        console.log(
          '[PinLogin] onSuccess access_code_version=',
          data.access_code_version
        );
      if (__DEV__)
        console.log(
          '[PinLogin] flag before setSession=',
      ...
      if (__DEV__)
        console.log(
          '[PinLogin] flag after setSession=',
          collectorPinLoginInProgress
        );

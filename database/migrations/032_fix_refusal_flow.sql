-- ============================================================
-- MIGRATION 032 — Fix refusal flow
--
-- Problèmes corrigés :
--   1. pickup_logs pas créé lors d'un refus → historique collector vide
--   2. Aucune notification parent lors d'un refus
--
-- La fonction validate_qr_and_create_pickup est remplacée par une
-- version qui, pour CHAQUE refus :
--   a) insère dans pickup_logs (status='denied') → historique collector
--   b) appelle insert_notification() pour le parent
-- ============================================================
...
DECLARE
  ...
  -- Helper : créer un log + notification refus puis retourner le JSON
  -- (pas de sous-fonctions en plpgsql, on factorise via un bloc inline)
...
BEGIN
  ...
  -- Nom de l'école pour les notifications
